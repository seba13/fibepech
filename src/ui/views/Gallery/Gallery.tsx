...
import { MediaLightbox , type MediaItem as MediaLightboxItem } from '../../components/MediaLightBox/MediaLightBox';
...
export const Gallery = () => {
  ...
  const lightboxItems: MediaLightboxItem[] = filtered.map((item) => ({
    type: item.type,
    src: item.src,
    poster: item.poster,
    alt: item.alt,
      ...
      <div className="container">
        ...
        <div className="gal-grid">
          {filtered.map((item, i) => (
            <div
              key={`${filter}-${i}`}
              className={`gi${item.wide ? " wide" : ""}${item.tall ? " tall" : ""}`}
              onClick={() => setLbIdx(i)}
            >
              {item.type === "video" ? (
                <>
